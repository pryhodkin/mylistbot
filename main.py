...
def handle_get(query):
    ...
    if text == "":
        text = "list is empty"
    result = telebot.types.InlineQueryResultArticle(int(query.id) + GenerateId(), 'get list', telebot.types.InputTextMessageContent(text))
    bot.answer_inline_query(query.id, [result], cache_time = 20)
...
def handle_get(query):
    result = telebot.types.InlineQueryResultArticle(query.id, f'del No. {classes.GetDelIndex(query.query)}', telebot.types.InputTextMessageContent(f"item No.{classes.GetDelIndex(query.query)} deleted"))
    bot.answer_inline_query(query.id, [result], cache_time = 20)
...
def handle_query(query):
    result = telebot.types.InlineQueryResultArticle(query.id, "new item:" + query.query, telebot.types.InputTextMessageContent("item: " + query.query + " added"))
    bot.answer_inline_query(query.id, [result], cache_time = 20)
